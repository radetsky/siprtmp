...
class Context(object):
    ...
    def rtmp_register(self, login=None, passwd='', display=None, rate="wideband"):
        ...
        if agent.sipserver:
            print aor
            sipname, ignore, sipaddr = aor.partition('@')
            if _debug: print '  ignore ', aor, ' use ', agent.sipserver 

        self._preferred = rate
        if agent.sipserver:
            print "rtmp_register: detected sipserver parameter. do not registering user at the server."
            print "just create and connect socket"
            print "SIP server: ", agent.sipserver
            addr = 'sip:rtmpgw@%s'%agent.sipserver
            print addr 
        else:
        ...
        try:
            ...
            if user.password and not agent.sipserver:
            ...
            if agent.sipserver: 
                if _debug: print '  connect successful', self.user.address 
        ...
        try:
            ...
            if self.user: # already a registered user exists
                if not self.session: # not already in a session, so create one
                    mydest = str(dest)
                    leftpart, ignore, ignore = mydest.partition('@');
                    dest = leftpart + '@' + agent.sipserver; 
                    print 'newdest=%r'%dest;
...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option('-s', '--sipserver', dest='sipserver', default='127.0.0.1', help="SIP server to use. Ignore another. For security and antiddos reason. Default '127.0.0.1'")
    ...
    try:
        ...
        if options.sipserver: agent.sipserver = options.sipserver
